...
public class ClientImportDialogController extends DefaultDialogController implements ImportStatusListener
{
  ...
  // - controllers
  private ImportCharacterSelectionController _selectionCtrl;
  ...
  private JPanel buildCommandsPanel()
  {
    ...
    _selectionCtrl=new ImportCharacterSelectionController(_startButton);
    ...
    JPanel selectionPanel=_selectionCtrl.getPanel();
  ...
  {
    LotroProcess selected=_selectionCtrl.getSelectedProcess();
    if (selected==null)
    {
      return;
    }
    final MemoryAccess memoryAccess=buildMemoryAccess(selected);
  ...
  private MemoryAccess buildMemoryAccess(LotroProcess selected)
  {
    WinInterface win=new WinInterface();
    boolean ok=win.start(selected);
    if (!ok)
    {
      return null;
    }
  ...
  private synchronized void updateUi(ImportStatusData data)
  {
    ...
    _selectionCtrl.setEnabled(enabled);
  }
